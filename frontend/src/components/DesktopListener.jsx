...
const DesktopListener = () => {
    ...
    // Debug states not strictly needed for stable PC version but harmless to keep internal logic consistent
    ...
    const createRecognition = () => {
        ...
        recognition.onresult = (event) => {
            for (let i = event.resultIndex; i < event.results.length; ++i) {
                ...
                setDebugTexto(transcript);

                if (result.isFinal) {
                    const fraseFinal = transcript.trim().toLowerCase();
    ...
    const reproducirRespuesta = async (audioUrl) => {
        ...
        const draw = () => {
            ...
            for (let i = 0; i < numPuntos; i++) {
                ...
                ctx.fillStyle = `rgb(${150 + volumen / 2}, ${50 - volumen}, ${200 - volumen / 2
                    })`;
                ctx.fill();
            }
        };
    ...
    const startVisualizer = async () => {
        ...
        const canvas = canvasRef.current;
        if (!canvas) console.error("❌ Canvas no encontrado en startVisualizer");
        const ctx = canvas?.getContext("2d");
        if (!ctx) console.error("❌ Contexto 2D no obtenido");

        const numPuntos = getNumPuntos();
        const espacio = canvas ? canvas.width / (numPuntos + 1) : 0;
        const centroY = canvas ? canvas.height / 2 : 0;

        console.log("✅ Iniciando visualizador. Puntos:", numPuntos, "Canvas:", canvas);
        ...
        const draw = () => {
            ...
            for (let i = 0; i < numPuntos; i++) {
                ...
                ctx.beginPath();
                ctx.arc(x, centroY, radio, 0, Math.PI * 2);
                ctx.fillStyle = `rgb(${100 + volumen}, ${150 + volumen / 2}, ${200 - volumen / 2
                    })`;
                ctx.fill();
            }
        };
